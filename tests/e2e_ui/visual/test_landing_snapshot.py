...
composer. The gate lives in ``.github/workflows/ui-snapshot.yml`` and runs on a
*pinned* ``ubuntu-24.04`` runner so the committed baseline and the PR comparison
are produced by the exact same renderer (screenshots differ across OSes; no diff
threshold can reconcile two rendering engines, so CI is the single source of
truth). To update the baseline, run that workflow via ``workflow_dispatch``,
download the ``ui-snapshot-baseline`` artifact, review the image, and commit it
-- see ``tests/e2e_ui/visual/README.md``.
